    ...
    extends IntTableFNTStrategy
{
    ...
    private class TableFNTThread
        extends Thread
    {
        public TableFNTThread(int length, boolean isInverse, ArrayAccess arrayAccess, int[] wTable, int[] permutationTable)
        {
    ...
    protected void multiplyElements(ArrayAccess arrayAccess, int startRow, int rows, int columns, int w, int scaleFactor)
        throws ApfloatRuntimeException
    {
        int[] data = arrayAccess.getIntData();
        int position = arrayAccess.getOffset();
        int tmp = modPow(w, (int) startRow);

        for (int i = 0; i < rows; i++)
        {
            int tmp2 = scaleFactor;
    ...
    protected void transformRows(int length, int count, boolean isInverse, ArrayAccess arrayAccess, int[] wTable, int[] permutationTable)
        throws ApfloatRuntimeException
    {
        ApfloatContext ctx = ApfloatContext.getContext();
    ...
    private void runThreads(int numberOfThreads, int length, int count, boolean isInverse, ArrayAccess arrayAccess, int[] wTable, int[] permutationTable)
        throws ApfloatRuntimeException
    {
